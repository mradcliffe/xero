...
class XeroQueryCacheTest extends XeroQueryTestBase {
  ...
  protected function setUp() {
    ...
    $this->listData = $this->getMockBuilder('\Drupal\Core\TypedData\Plugin\DataType\ItemList')
    ...
    $this->typedDataManager->expects($this->any())
      ->method('create')
      ->with($this->listDefinition, $data)
      ->will($this->returnValue($this->listData));
  ...
  public function testGetSetCache() {
    ...
    $data = $this->query->getCache('xero_user');
    $this->assertEquals('Smith', $data->get(0)->get('LastName')->getValue());
  }
